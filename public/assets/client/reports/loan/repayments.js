...
function disbursementsChartsReport(principal, interest, total) {
    ...
    var principalCollected = principal;
    ...
    var interestCollected = interest;
    ...
    var totalCollections = total;

    /* area chart */
    var areaOptions = {
        series: [{
            name: 'Interest Collected',
            data: monthlyDisbursementsNumber(interestCollected)
        }, {
            name: 'Principal Collected',
            data: monthlyDisbursementsNumber(principalCollected),
        }, {
            name: 'Total Collections',
            data: monthlyDisbursementsNumber(totalCollections),
        }],
        chart: {
            height: 320,
            type: 'area'
...
function searchDisbursementLoans() {
    ...
    // JS  principal collected data objects
    ...
    // JS interest collected data objects
    var interestObjects = {
        menu: "",
        year: year,
        start_date: start_date,
    ...
    var url = baseURL + "/client/reports/view/report/repayments";
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            $("#loan_repayments").html('<div class="text-center">' + '<i class="fa fa-spinner fa-pulse fa-4x fa-fw"></i></div>');
        ...
        success: function (results) {
            disbursementsChartsReport(principalObjects, interestObjects, totalObjects);
            // loan repayment counter
            var loanRepaymentDataTable =
                '<div class="table-responsive"><table id="loanRepayment" class="table table-sm table-striped table-hover"' +
                'cellspacing="0" width="100%"><tbody></tbody>' +
                '<tfoot>' +
                '<tr>' +
                '<th>Total</th>' +
                '<th>' + results.repayments.principal + '</th>' +
                '<th>' + results.repayments.interest + '</th>' +
                '<th>' + results.repayments.total + '</th>' +
                '</tr>' +
                '</tfoot></table></div>';

            $("#loan_repayments").html(loanRepaymentDataTable);

            $("#loanRepayment").DataTable({
                data: results.repayments.loans,
                bPaginate: true,
                bLengthChange: true,
                language: {
                    paginate: {
...
function monthlyDisbursementsNumber(data) {
    ...
    $.ajax({
        url: "/client/reports/view/report/repayments",
        async: false,
        type: "POST",
        dataType: "JSON",
        data: {
        ...
        success: function (data) {
            // if (data.collections.principal > 0) {
                if (menu == 'principal') {
                    data.collections.principal.forEach(function (value, index) {
                        total.push(value);
                    });
                    // for (let index in data.collections.principal) {
                    //   const element = data.collections.principal[index];
                    //   total.push(element)
                ...
                if (menu == 'interest') {
                    data.collections.interest.forEach(function (value, index) {
                        total.push(value);
                    });
                    // for (let index in data.collections.interest) {
                    //   const element = data.collections.interest[index];
                    //   total.push(element)
                ...
                if (menu == 'total') {
                    data.collections.total.forEach(function (value, index) {
                        total.push(value);
                    });
                    // for (let index in data.collections.total) {
                    //   const element = data.collections.total[index];
                    //   total.push(element)
