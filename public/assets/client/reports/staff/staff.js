...
if (userPermissions.includes('export' + title) || userPermissions === '"all"') {
    buttonsConfig.push(
        ...
        {
            extend: 'print',
            title: "<h3 class='text-center text-bold'>" + businessName + "</h3><h4 class='text-center text-bold'>Staff " + title + " Information</h4><h5 class='text-center'>Printed On " + new Date().getHours() + " : " + new Date().getMinutes() + " " + new Date().toDateString() + "</h5>",
...
function filterStaffsAccount() {
    ...
    $.ajax({
        ...
        dataType: "JSON",
        beforeSend: function () {
            $("#staff_counter").html('<div class="text-center">' + '<i class="fa fa-spinner fa-pulse fa-4x fa-fw"></i></div>');
            // load the spinner where staff table is
            $("#staff_table").html('<div class="text-center">' + '<i class="fa fa-spinner fa-pulse fa-4x fa-fw"></i></div>');
        },
        success: function (results) {
            ...
            var staffCounterDataTable =
                ...
                '<tr>' +
                '<th>Total Staff</th>' +
                '<th>' + results.yearlyActiveStaffTotal + '</th>' +
                '<th>' + results.yearlyInactiveStaffTotal + '</th>' +
                '<th>' + results.yearlyStaffTotal + '</th>' +
                '</tr>' +
                '</tfoot></table></div>';
...
function viewStaffAccount(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.photo && (imageExists('/uploads/staffs/admins/passports/' + data.photo))) {
                $('#photo-preview div').html('<img src="/uploads/staffs/admins/passports/' + data.photo + '" class="img-fluid thumbnail"  style="width: 190px; height: 190px;">');
            } else {
                $('#photo-preview div').html('<img src="/assets/dist/img/nophoto.jpg" class="img-fluid thumbnail"  style="width: 190px; height: 190px;">');
            }
            if (data.id_photo && (imageExists('/uploads/admins/ids/' + data.id_photo))) {
                $('#id-preview div').html('<img src="/uploads/admins/ids/' + data.id_photo + '" class="img-fluid thumbnail"  style="width: 200px; height: 180px;">');
            } else {
                $('#id-preview div').html('<img src="/assets/dist/img/id.jpg" class="img-fluid thumbnail"  style="width: 200px; height: 180px;">');
            }
            if (data.signature && (imageExists('/uploads/staffs/admins/signatures/' + data.signature))) {
                $('#signature-preview div').html('<img src="/uploads/staffs/admins/signatures/' + data.signature + '" alt="Sign preview" id="preview-sign" class="img-fluid thumbnail" style="width: 140px; height: 140px;">');
            } else {
...
function viewStaffAccountPhoto(id) {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.photo && (imageExists("/uploads/staffs/admins/passports/" + data.photo))) {
                $('#photo-preview div').html('<img src="/uploads/staffs/admins/passports/' + data.photo + '" class="img-fluid thumbnail">');
            } else {
