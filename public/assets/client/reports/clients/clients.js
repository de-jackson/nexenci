...
if (userPermissions.includes('export' + title) || userPermissions === '"all"') {
    buttonsConfig.push(
        ...
        {
            extend: 'print',
            title: "<h3 class='text-center text-bold'>" + businessName + "</h3><h4 class='text-center text-bold'>Clients " + title + " Information</h4><h5 class='text-center'>Printed On " + new Date().getHours() + " : " + new Date().getMinutes() + " " + new Date().toDateString() + "</h5>",
...
function filterClientsAccount() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            $("#clients_counter").html('<div class="text-center">' + '<i class="fa fa-spinner fa-pulse fa-4x fa-fw"></i></div>');
            // load spinner where there is clients table report
            $("#clients_table").html('<div class="text-center">' + '<i class="fa fa-spinner fa-pulse fa-4x fa-fw"></i></div>');
        },
        success: function (results) {
            ...
            var dataTableClients =
                ...
                '<tr>' +
                '<th>Total Clients</th>' +
                '<th>' + results.yearlyActiveClientsTotal + '</th>' +
                '<th>' + results.yearlyInactiveClientsTotal + '</th>' +
                '<th>' + results.yearlyClientsTotal + '</th>' +
                '</tr>' +
                '</tfoot></table></div>';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.photo && (imageExists('/uploads/clients/passports/' + data.photo))) {
                $('#photo-preview div').html('<img src="/uploads/clients/passports/' + data.photo + '" class="img-fluid thumbnail"  style="width: 190px; height: 190px;">');
            } else {
                $('#photo-preview div').html('<img src="/assets/dist/img/nophoto.jpg" class="img-fluid thumbnail"  style="width: 190px; height: 190px;">');
            }
            if (data.signature && (imageExists('/uploads/clients/signatures/' + data.signature))) {
                $('#signature-preview div').html('<img src="/uploads/clients/signatures/' + data.signature + '" alt="Sign preview" id="preview-sign" class="img-fluid thumbnail" style="width: 140px; height: 140px;">');
            } else {
