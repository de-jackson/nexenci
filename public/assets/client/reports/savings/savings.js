    ...
    buttonsConfig.push(
        {
            ...
            filename: 'Clients Savings ' + title + ' Information',
            extension: '.xlsx',
        },
        {
            extend: 'pdf',
            className: 'btn btn-sm btn-danger',
            titleAttr: 'Export Clients Savings ' + title + ' To PDF',
        ...
        },
        {
            extend: 'csv',
            className: 'btn btn-sm btn-success',
            titleAttr: 'Export Clients Savings ' + title + ' To CSV',
        ...
        },
        {
            ...
            titleAttr: 'Copy Clients Savings ' + title + ' Information',
            text: '<i class="fas fa-copy"></i>',
        },
        {
            extend: 'print',
            title: "<h3 class='text-center text-bold'>" + businessName + "</h3><h4 class='text-center text-bold'>Clients Savings " + title + " Information</h4><h5 class='text-center'>Printed On " + new Date().getHours() + " : " + new Date().getMinutes() + " " + new Date().toDateString() + "</h5>",
            ...
            titleAttr: 'Print Clients Savings ' + title + ' Information',
...
$(function () {
    ...
    filterClientSavingsReport();

    // auto load the savings chart
    savingsCharts(data, creditData, totalAmount);

});

function savingsCharts(jsDebitDataObject, jsCreditDataObject, totalAmount) {
    /* ChartJS
     * -------
    ...
    // Fetch all the keys(variables) from the object
    var totalAmountKeys = Object.keys(totalAmount);

    //console.log(totalAmountKeys) // [entry_status, year, start_date etc]

    // Iterate over each key to update entry status to debit
    totalAmountKeys.forEach((totalAmountElement) => {
        // check whether the element is entry_status to update its value to debit
        if (totalAmountElement == "entry_status") {
            jsCreditDataObject[totalAmountElement] = "total";
        }
    });

    var totalSavingsAmount = totalAmount;
    ...
    var areaOptions = {
        series: [{
            ...
            name: 'Clients Savings Transactions',
            data: monthly_totals(totalSavingsAmount),
        }],
        chart: {
            height: 320,
            type: 'area'
        ...
        title: {
            text: 'Monthly Clients Transactions Report - Area Chart',
            align: 'left',
            style: {
                fontSize: '13px',
                fontWeight: 'bold',
        ...
        yaxis: {
            title: {
                text: 'Clients Savings Transactions',
                style: {
                    color: "#8c9097",
                }
            },
    ...
    var lineOptions = {
        ...
        title: {
            text: 'Monthly Clients Transaction Report - Line Graph',
            align: 'left',
            style: {
                fontSize: '13px',
                fontWeight: 'bold',
    ...
    var columnOptions = {
        ...
        colors: ["#23b7e5", "#f5b849", "#845adf"],
        title: {
            text: 'Monthly Clients Savings Transaction Report - Column Chart',
            align: 'left',
            style: {
                fontSize: '13px',
                fontWeight: 'bold',
    ...
    var radarOptions = {
        ...
        title: {
            text: 'Monthly Clients Savings Transactions Report - Radar Chart',
            align: 'left',
            style: {
                fontSize: '13px',
                fontWeight: 'bold',
...
function savingEntryYears(account_type_id = null) {
    $.ajax({
        url: "/client/reports/years-of-entries/12",
        type: "GET",
        dataType: "JSON",
...
function transactionEntryTypes(entry_typeId = null) {
    $.ajax({
        url: '/client/reports/transactions/12',
        type: "POST",
        dataType: "JSON",
...
function filterClientSavingsReport() {
    ...
    // JS  total savings data objects
    var totalSavingsData = {
        entry_status: "",
        year: year,
        start_date: start_date,
        end_date: end_date,
    ...
    var url = baseURL + "/client/reports/view/report/savings";
    ...
    $.ajax({
        ...
        success: function (results) {
            ...
            savingsCharts(data, creditObject, totalSavingsData);
            ...
            var dataTable =
                ...
                '<tr>' +
                '<th>Total Amount</th>' +
                '<th>' + results.yearlyTotalCredit + '</th>' +
                '<th>' + results.yearlyTotalDebit + '</th>' +
                '<th>' + results.yearlyTotal + '</th>' +
                '</tr>' +
                '</tfoot></table></div>';
            ...
            var dataTableClient =
                ...
                '<tr>' +
                '<th>Total</th>' +
                '<th>' + results.yearlyDebitClientsNumber + '</th>' +
                '<th>' + results.yearlyDebitClientsNumber + '</th>' +
                '<th>' + results.yearlyClientsNumber + '</th>' +
                '</tr>' +
                '</tfoot></table></div>';
...
function monthly_totals(data) {
    const total = [];
    $.ajax({
        url: "/client/reports/view/report/savings",
        async: false,
        type: "POST",
        dataType: "JSON",
        data: {
