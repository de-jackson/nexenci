  ...
  buttonsConfig.push(
    ...
    },
    {
      ...
      title:
        ...
        businessName +
        "</h3><h4 class='text-center text-bold'>Products " +
        title +
...
function loanApplicationsChartsReport(disbursed, approved, applications) {
  ...
  var areaOptions = {
    series: [
      {
        name: "Approved Loan Applications",
        data: monthlyTotalLoanApplication(approvedApplications),
      },
      {
        name: "Disbursed Loan Applications",
...
function yearsInStaffsTable() {
  $.ajax({
    url: "/admin/reports/module/types/loanapplicationyears",
    type: "GET",
    dataType: "JSON",
...
function loadLoanApplicationLevels() {
  $.ajax({
    url: "/admin/reports/module/types/loanapplicationlevels",
    type: "POST",
    dataType: "JSON",
    success: function (data) {
      $("select#application_level").html(
      ...
      if (keys.length > 0) {
        for (let index in keys) {
          $("<option />")
            .val(keys[index])
            .text(keys[index])
            .appendTo($("select#application_level"));
        }
      } else {
        $("select#application_level").html(
          '<option value="">No Application Level</option>'
        );
      }
    },
    error: function (err) {
...
function loadLoanApplicationActions() {
  $.ajax({
    url: "/admin/reports/module/types/loanapplicationactions",
    type: "POST",
    dataType: "JSON",
    success: function (data) {
      $("select#application_action").html(
        '<option value="">Select Application Action</option>'
      );
      // convert object to key's array
      const keys = Object.keys(data);
      if (keys.length > 0) {
        for (let index in keys) {
          $("<option />")
            .val(keys[index])
            .text(keys[index])
            .appendTo($("select#application_action"));
        }
      } else {
        $("select#application_action").html(
          '<option value="">No Application Action</option>'
        );
      }
    },
    error: function (err) {
...
function loadLoanApplicationStatus() {
  $.ajax({
    url: "/admin/reports/module/types/loanapplicationstatus",
    type: "POST",
    dataType: "JSON",
    success: function (data) {
      $("select#application_status").html(
      ...
      if (keys.length > 0) {
        for (let index in keys) {
          $("<option />")
            .val(keys[index])
            .text(keys[index])
            .appendTo($("select#application_status"));
        }
      } else {
        $("select#application_status").html(
          '<option value="">No Application Status</option>'
        );
      }
    },
    error: function (err) {
...
function searchLoanApplications() {
  ...
  // JS  disbursed loan applications data objects
  ...
  // JS  Approved loan applications data objects
  var approvedObjects = {
    year: year,
    start_date: start_date,
    end_date: end_date,
  ...
  var url = baseURL + "/admin/reports/module/get-report-type/loanapplications";
  ...
  $.ajax({
    ...
    success: function (results) {
      ...
      var loanApplicationsCounterDataTable =
        ...
        "<th>" +
        results.yearlytotalpending +
        "</th>" +
        "<th>" +
        results.yearlytotalprocessing +
        "</th>" +
        "<th>" +
        results.yearlytotaldeclined +
        "</th>" +
        "<th>" +
        results.yearlytotalapproved +
        "</th>" +
        "<th>" +
        results.yearlytotaldisbursed +
        "</th>" +
        "<th>" +
        results.yearlytotalcancelled +
        "</th>" +
        "<th>" +
        results.yearlytotalapplications +
        "</th>" +
        "</tr>" +
        "</tfoot></table></div>";
      ...
      $("#applicationCounter").DataTable({
        ...
        columns: [
          ...
          { title: "Total Number of Loan Applications" },
        ],
        // dom: "lBfrtip",
...
function monthlyTotalLoanApplication(data) {
  const total = [];
  $.ajax({
    url: "/admin/reports/module/get-report-type/loanapplications",
    async: false,
    type: "POST",
    dataType: "JSON",
    data: {
