var save_method;
var tableId = "employeesTable";
// dataTables url
var tableDataUrl = "/admin/staff/generate-employees/" + id;
...
function add_employee() {
  ...
  $("#importRow").hide();
  $("#formRow").show(); // show bootstrap modal
...
function import_staff() {
  ...
  $(".modal-title").text("Import New Employees");
  $("#importRow").show();
  $("#formRow").hide();
  $("#export").hide();
  $('[name="id"]').val(0);
  $('[name="mode"]').val("imported");
  $('[name="account_type"]').val("Employee");
}
// view record
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (
        data.id_photo_front &&
        imageExists(
          "/uploads/staffs/employees/ids/front/" + data.id_photo_front
        )
      ) {
        $("#id-previewFront div").html(
          '<img src="/uploads/staffs/employees/ids/front/' +
            data.id_photo_front +
      ...
      if (
        data.id_photo_back &&
        imageExists("/uploads/staffs/employees/ids/back/" + data.id_photo_back)
      ) {
        $("#id-previewBack div").html(
          '<img src="/uploads/staffs/employees/ids/back/' +
            data.id_photo_back +
            '" class="img-fluid thumbnail"  style="width: 200px; height: 180px;">'
        );
      } else {
...
// view employee photo
function view_employee_photo(id) {
  $.ajax({
    url: "/admin/staff/employee/" + id,
    type: "GET",
    dataType: "JSON",
...
function save_employee() {
  ...
  if (save_method == "add") {
    ...
    url = "/admin/staff/edit-employee/" + eID;
  }
...
function delete_employee(id, name) {
  Swal.fire({
    title: "Are you sure?",
    text: "You won't be able to recover" + name + " employee!",
    icon: "warning",
    showCancelButton: true,
    confirmButtonColor: "#3085d6",
    cancelButtonColor: "#d33",
...
function bulk_deleteEmployees() {
  ...
  if (list_id.length > 0) {
      ...
      .then((result) => {
        if (result.isConfirmed) {
          $.ajax({
            ...
            url: "/admin/staff/bulk-deleteEmployees",
            dataType: "JSON",
            success: function (data) {
              if (data.status && data.error == null) {
                Swal.fire("Success!", data.messages, "success");
    ...
    Swal.fire("Sorry!", "No employee selected....", "error");
    reload_table(tableId);
  }
}
