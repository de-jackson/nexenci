...
class Modules extends MasterController
{
    ...
    public function reportViewType($menu)
    {
        if ($this->checkPermissions($this->userPermissions, $this->menuItem)) {
            ...
            switch (strtolower($menu)) {
                ...
                case 'savings':
                    $view = 'admin/reports/modules/savings/savings';
                    break;

                case 'loanproducts':
                    $view = 'admin/reports/modules/loan/products';
                    break;

                case 'loanapplications':
                    $view = 'admin/reports/modules/loan/applications';
                    break;

                case 'disbursements':
                    $view = 'admin/reports/modules/loan/disbursements';
                    break;
                ...
                case 'repayments':
                    $view = 'admin/reports/modules/loan/repayments';
                    break;

                case 'arrears':
                    $view = 'admin/reports/modules/loan/arrears';
                    break;
    ...
    public function types($menu)
    {
        switch (strtolower($menu)) {
            case 'title':
                $results = $this->settings->generateTitles();
                break;
            ...
            case 'interest_period':
                $results = $this->settings->loanInterestPeriods();
                break;
            ...
            case 'loan_frequency':
                $results = $this->settings->getLoanFrequencyDurations();
                break;
            ...
            case 'entries':
                # generate the reports from the entries table
                $frequency = 'weekly';
                $results = $this->report->getEntriesReport($frequency);
                break;

            case 'accounts':
                $results = $this->account->where(['account_id' => null, 'status' => '1'])->findAll();
                break;

            case 'categories':
                $results = $this->blog->where([
                    'blog_id' => NULL
                ])->orderBy('id', 'desc')->findAll();
                break;
    ...
    public function getReportType($menu)
    {
        switch (strtolower($menu)) {
            ...
            case 'dashboard':
                # generate savings report
                $reportResults = $this->loanOfficerReport();
                break;
    ...
    public function savingsReport()
    {
        ...
        # from the savingsResults, pick only the monthlyEntries
        foreach ($savingsResults["monthlyEntries"] as $key => $savingResults) {
            # check the savings collection report existence
            if (count($savingResults) > 1) {
                $debitAmount = $creditAmount = $amount = 0;
                # set the number of savings entries
                $clientsTotal = $debitClientsTotal = $creditClientsTotal = [];
                foreach ($savingResults as $k2 => $v2) {
                    if ($v2) {
                        ...
                        # check whether the entry status is credit
                        if (strtolower($v2['status']) == "credit") {
                            # check whether the auto deduction charges exist
                            if ($v2['parent_id']) {
                                # remove the deduction charges
                                $amount -= $v2['amount'];
                            } else {
                                # add the total deposit
                                $amount += $v2['amount'];
                            }
                            # credit total client amount
                            $creditAmount += $v2['amount'];
                            # count the total number clients with credit
                            $creditClientsTotal[] = $v2['client_id'];
                        }
                ...
                # calculate the total amount of credit, debit
                $results[$key][] = ($creditAmount);
                $results[$key][] = ($debitAmount);
                $results[$key][] = ($amount);
                # count unique number of clients
                $results[$key][] = count(array_unique(($creditClientsTotal)));
                $results[$key][] = count(array_unique(($debitClientsTotal)));
        ...
        $balanceBroughtForward = 0; // Set the default balance brought forward value.
        # Calculate initial balance brought forward
        $balanceBroughtForward = $this->report->calculateBalanceBroughtForward();

        // Add the "Balance Brought Forward" row
        $entriesResults[] = [
            $sno,
            $start_date, // No date for balance brought forward
            '<b>BALANCE BROUGHT FORWARD</b>',
            '-', // No account number for balance brought forward
            number_format($balanceBroughtForward, 2),
            '-',
            '-',
            number_format($balanceBroughtForward, 2)
        ];
        ...
        $sno++; // Increment the serial number for subsequent rows.

        # from the savingsResults, pick only the entries results
        foreach ($savingsResults["entries"] as $key => $entry) {
            # check the account side
            ...
            $row[] = $sno;
            $row[] = $entry['date'];
            $row[] = strtoupper($entry['name']);
            $row[] = $entry['account_no'];
            $row[] = number_format($entry['amount'], 2);
            $row[] = ucwords($entry['particular_name']);
            $row[] = ucwords($entry['type']);
    ...
    public function loanApplicationsReport()
    {
        ...
        foreach ($applicationResults["loanApplicationsResults"] as $key => $loanapplication) {
            ...
            $row[] = '
                <div class="text-center">
                    <a href="/admin/application/info/' . $loanapplication['id'] . '" title="View ' . ucwords(strtolower($loanapplication['name'])) . '" class="' . $text . '"><i class="fas fa-eye"></i></a>
                </div>
            ';
    ...
    public function loanDisbursementReport()
    {
        ...
        foreach ($disbursementResults["loanDisbursementResults"] as $key => $disbursement) {
            ...
            $row[] = '
                <div class="text-center">
                    <a href="/admin/disbursement/info/' . $disbursement['id'] . '" title="View ' . ucwords(strtolower($disbursement['name'])) . '" class="' . $text . '"><i class="fas fa-eye"></i></a>
                </div>
            ';
    ...
    public function loanOfficerReport()
    {
        $start_date = $this->request->getVar('start_date');
        $end_date = $this->request->getVar('end_date');
        ...
        # get the clients report
        $report1 = $this->report->clientsReport();
        ...
        $report = array_merge($summary, $report1);
