...
class Loans extends MainController
{
            ...
            ->add('actions', function ($application) {
                ...
                */
                ...
                if ($application_status == "approved") {
                    ...
                    if (($this->userPermissions == 'all') || (in_array('create_loansDisbursement', $this->userPermissions))) {
                        // $actions .= '
                        //     <div class="dropdown-divider"></div>
    ...
    {
        if (isset($class)) {
            $where = ['disbursements.client_id' => $this->userRow['id'], 'disbursements.deleted_at' => null, 'disbursements.class' => ucfirst($class)];
        } else {
            $where = ['disbursements.client_id' => $this->userRow['id'], 'disbursements.deleted_at' => null];
        }
        ...
        $disbursements = $this->disbursement->select('clients.id as client_id, clients.name, loanproducts.product_name, disbursements.disbursement_code,disbursements.principal, disbursements.actual_interest, disbursements.actual_repayment, disbursements.total_balance, disbursements.total_collected, disbursements.actual_installment, disbursements.class, disbursements.days_remaining, disbursements.id')
            ->join('clients', 'clients.id = disbursements.client_id', 'left')->join('loanproducts', 'loanproducts.id = disbursements.product_id', 'left')
            ->where($where);
            ...
            ->add('action', function ($disbursement) {
                ...
                switch (strtolower($disbursement->class)) {
                    case "running":
                        ...
                        if (($this->userPermissions == 'all') || (in_array('create_transactionsRepayments', $this->userPermissions))) {
                            $actions .= '
                                <div class="dropdown-divider"></div>
